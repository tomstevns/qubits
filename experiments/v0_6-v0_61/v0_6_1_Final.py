#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
v0_6.1_Final.py — Systematic Signature Detection (v0_6.1) with evidence checks

Adds 3 "scientifically convincing but lightweight" checks without changing the core logic:
1) Robustness sweep: threshold, epsilon, entropy-binning (on a subset for speed)
2) Enrichment test: stable set (top 1% / 5% scores) vs overall + bootstrap CI
3) Hold-out replication: split seeds in half, verify top signature families replicate

Local run. Writes console + UTF-8 log file.
"""

from __future__ import annotations
import argparse, math, os, sys
from dataclasses import dataclass
from typing import Dict, List, Sequence, Tuple
import numpy as np

try:
    from qiskit.quantum_info import SparsePauliOp
except Exception as e:
    raise RuntimeError("Requires qiskit (SparsePauliOp). Install: pip install qiskit") from e


class Tee:
    def __init__(self, filename: str, mode: str = "w", encoding: str = "utf-8"):
        self.filename, self.mode, self.encoding = filename, mode, encoding
        self._file = None
        self._stdout = None

    def __enter__(self):
        self._stdout = sys.stdout
        self._file = open(self.filename, self.mode, encoding=self.encoding, errors="replace")
        sys.stdout = self
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        try:
            sys.stdout = self._stdout
        finally:
            if self._file:
                try:
                    self._file.flush()
                finally:
                    self._file.close()
        return False

    def write(self, obj):
        s = str(obj)
        if self._stdout:
            self._stdout.write(s)
        if self._file:
            self._file.write(s)

    def flush(self):
        if self._stdout:
            self._stdout.flush()
        if self._file:
            self._file.flush()


def shannon_entropy_base2(prob: np.ndarray, eps: float = 1e-12) -> float:
    p = np.clip(prob, eps, 1.0)
    p = p / np.sum(p)
    return float(-np.sum(p * np.log2(p)))


def build_random_hamiltonian_sparse(n_qubits: int, num_terms: int, seed: int) -> SparsePauliOp:
    rng = np.random.default_rng(seed)
    paulis = ["I", "X", "Y", "Z"]
    labels, coeffs = [], []
    for _ in range(num_terms):
        labels.append("".join(rng.choice(paulis, size=n_qubits)))
        coeffs.append(complex(rng.uniform(-1.0, 1.0)))
    return SparsePauliOp(labels, coeffs=np.asarray(coeffs, dtype=complex))


def diagonalize(H: SparsePauliOp) -> Tuple[np.ndarray, np.ndarray]:
    mat = H.to_matrix()
    evals, evecs = np.linalg.eigh(mat)
    return evals.real, evecs  # evecs columns


def evolve(evals: np.ndarray, evecs: np.ndarray, t: float, psi0: np.ndarray) -> np.ndarray:
    phases = np.exp(-1j * evals * t)
    a = evecs.conj().T @ psi0
    return evecs @ (phases * a)


def dominant_support(prob: np.ndarray, prob_threshold: float) -> np.ndarray:
    return np.sort(np.where(prob >= prob_threshold)[0])


def signature_key(dom_count: int, ent_i: float, ent_j: float, leakage: float, ent_bin: float, leak_bin: float) -> Tuple[int, int, int, int]:
    ei = int(round(ent_i / ent_bin))
    ej = int(round(ent_j / ent_bin))
    if ej < ei:
        ei, ej = ej, ei
    lk = int(round(leakage / leak_bin))
    return (int(dom_count), ei, ej, lk)


def score_interestingness(leakage_raw: float, dom_count: int, d: int) -> float:
    baseline = 1.0 - (dom_count / float(d))
    if baseline <= 1e-12:
        return 0.0
    ratio = max(0.0, min(1.0, leakage_raw / baseline))
    return 1.0 - ratio


@dataclass(frozen=True)
class Candidate:
    seed: int
    pair: Tuple[int, int]
    dE: float
    dom_count: int
    entropy_i: float
    entropy_j: float
    leakage_raw: float
    score: float
    sig: Tuple[int, int, int, int]


def candidates_for_seed(
    seed: int,
    n_qubits: int,
    num_terms: int,
    epsilon: float,
    prob_threshold: float,
    ent_bin: float,
    leak_bin: float,
    t: float,
) -> List[Candidate]:
    H = build_random_hamiltonian_sparse(n_qubits, num_terms, seed)
    evals, evecs = diagonalize(H)
    d = 2 ** n_qubits
    out: List[Candidate] = []

    # neighbor scan only (scientifically aligned with "near-degenerate")
    for i in range(len(evals) - 1):
        j = i + 1
        dE = float(abs(evals[j] - evals[i]))
        if dE >= epsilon:
            continue

        psi_i, psi_j = evecs[:, i], evecs[:, j]
        prob_i, prob_j = np.abs(psi_i) ** 2, np.abs(psi_j) ** 2
        ent_i, ent_j = shannon_entropy_base2(prob_i), shannon_entropy_base2(prob_j)

        supp = np.unique(np.concatenate([dominant_support(prob_i, prob_threshold),
                                         dominant_support(prob_j, prob_threshold)]))
        dom_count = int(len(supp))
        if dom_count <= 0 or dom_count >= d:
            continue

        psi0 = np.zeros((d,), dtype=complex)
        psi0[supp] = 1.0
        psi0 /= np.linalg.norm(psi0)

        psi_t = evolve(evals, evecs, t, psi0)
        prob_t = np.abs(psi_t) ** 2
        stay = float(np.sum(prob_t[supp]))
        leakage_raw = max(0.0, min(1.0, 1.0 - stay))

        score = score_interestingness(leakage_raw, dom_count, d)
        sig = signature_key(dom_count, ent_i, ent_j, leakage_raw, ent_bin, leak_bin)

        out.append(Candidate(seed, (i, j), dE, dom_count, float(ent_i), float(ent_j), float(leakage_raw), float(score), sig))

    return out


def run_scan(
    seeds: Sequence[int],
    n_qubits: int,
    num_terms: int,
    epsilon: float,
    prob_threshold: float,
    ent_bin: float,
    leak_bin: float,
    t: float,
) -> List[Candidate]:
    all_c: List[Candidate] = []
    for sd in seeds:
        all_c.extend(candidates_for_seed(sd, n_qubits, num_terms, epsilon, prob_threshold, ent_bin, leak_bin, t))
    return all_c


def top_signatures_by_enrichment(candidates: Sequence[Candidate], top_fraction: float, top_k: int = 10):
    if not candidates:
        return []
    scores = np.array([c.score for c in candidates], dtype=float)
    cutoff = np.quantile(scores, 1.0 - top_fraction)
    stable = [c for c in candidates if c.score >= cutoff]
    if not stable:
        return []

    overall: Dict[Tuple[int, int, int, int], int] = {}
    stable_c: Dict[Tuple[int, int, int, int], int] = {}
    for c in candidates:
        overall[c.sig] = overall.get(c.sig, 0) + 1
    for c in stable:
        stable_c[c.sig] = stable_c.get(c.sig, 0) + 1

    N, Ns = len(candidates), len(stable)
    ranked = []
    for sig, ca in overall.items():
        cs = stable_c.get(sig, 0)
        p_all = ca / N
        if p_all <= 0:
            continue
        p_stable = cs / Ns
        ranked.append((sig, float(p_stable / p_all), int(cs), int(ca)))
    ranked.sort(key=lambda x: x[1], reverse=True)
    return ranked[:top_k]


def bootstrap_enrichment_ci(candidates: Sequence[Candidate], sig: Tuple[int, int, int, int], top_fraction: float, n_boot: int, rng_seed: int = 123):
    rng = np.random.default_rng(rng_seed)
    N = len(candidates)
    if N == 0:
        return (float("nan"), float("nan"), float("nan"))

    vals = []
    for _ in range(n_boot):
        idx = rng.integers(0, N, size=N)
        sample = [candidates[i] for i in idx]
        scores = np.array([c.score for c in sample], dtype=float)
        cutoff = np.quantile(scores, 1.0 - top_fraction)
        stable = [c for c in sample if c.score >= cutoff]
        if not stable:
            continue
        ca = sum(1 for c in sample if c.sig == sig)
        if ca == 0:
            continue
        cs = sum(1 for c in stable if c.sig == sig)
        p_all = ca / len(sample)
        p_stable = cs / len(stable)
        vals.append(p_stable / p_all)

    if len(vals) < 10:
        return (float("nan"), float("nan"), float("nan"))
    arr = np.array(vals, dtype=float)
    return (float(arr.mean()), float(np.quantile(arr, 0.025)), float(np.quantile(arr, 0.975)))


def summarize_consistency(sweep_top: Dict[Tuple[float, float, float], List[Tuple[int, int, int, int]]], top_k: int = 10):
    sets = [set(v[:top_k]) for v in sweep_top.values() if v]
    if len(sets) <= 1:
        return (float("nan"), [])
    jac = []
    for i in range(len(sets)):
        for j in range(i + 1, len(sets)):
            a, b = sets[i], sets[j]
            inter, union = len(a & b), len(a | b)
            jac.append(inter / union if union else 0.0)
    avg = float(np.mean(jac)) if jac else float("nan")
    freq: Dict[Tuple[int, int, int, int], int] = {}
    for s in sets:
        for sig in s:
            freq[sig] = freq.get(sig, 0) + 1
    return avg, sorted(freq.items(), key=lambda x: x[1], reverse=True)


def print_basic_stats(cands: Sequence[Candidate], n_qubits: int):
    if not cands:
        print("No candidates retained.")
        return
    scores = np.array([c.score for c in cands], dtype=float)
    leak = np.array([c.leakage_raw for c in cands], dtype=float)
    dom = np.array([c.dom_count for c in cands], dtype=int)
    print(f"Retained candidates: {len(cands)}")
    print(f"Score: mean={scores.mean():.3f}, median={np.median(scores):.3f}, max={scores.max():.3f}")
    print(f"Leakage_raw: mean={leak.mean():.3f}, median={np.median(leak):.3f}, min={leak.min():.3f}")
    print("dom_count distribution:")
    for k in sorted(set(dom.tolist())):
        n = int(np.sum(dom == k))
        print(f"  dom_count={k}: {100.0*n/len(dom):.1f}% (n={n})")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--seeds", type=int, default=50000)
    ap.add_argument("--seed_offset", type=int, default=0)
    ap.add_argument("--n_qubits", type=int, default=3)
    ap.add_argument("--num_terms", type=int, default=5)
    ap.add_argument("--t", type=float, default=1.0)
    ap.add_argument("--leak_bin", type=float, default=0.05)
    ap.add_argument("--bootstrap", type=int, default=300)
    ap.add_argument("--output", type=str, default="v0_6_1_Final_output.txt")
    args = ap.parse_args()

    n_qubits = args.n_qubits
    num_terms = args.num_terms
    d = 2 ** n_qubits
    leak_bin = float(args.leak_bin)
    t = float(args.t)

    # sweep knobs
    thresholds = [0.20, 0.25, 0.30]
    epsilons = [0.02, 0.05, 0.10]
    ent_bins = [0.05, 0.10]

    # nominal
    nominal_th = 0.25
    nominal_eps = 0.05
    nominal_ent_bin = 0.10

    seeds = list(range(args.seed_offset, args.seed_offset + args.seeds))
    half = len(seeds) // 2
    seeds_A, seeds_B = seeds[:half], seeds[half:]

    with Tee(args.output):
        print("=== v0_6.1_Final: Evidence Checks for Systematic Signature Families ===")
        print(f"Qubits: {n_qubits} (d={d}) | terms={num_terms} | seeds={len(seeds)} | t={t}\n")

        # 1) robustness sweep (subset for speed)
        sweep_N = min(5000, len(seeds))
        sweep_seeds = seeds[:sweep_N]
        print("=== 1) Robustness Sweep ===")
        print(f"Using first {sweep_N} seeds for sweep (speed). Full run used for enrichment + hold-out.\n")
        sweep_top: Dict[Tuple[float, float, float], List[Tuple[int, int, int, int]]] = {}

        for th in thresholds:
            for eps in epsilons:
                for eb in ent_bins:
                    c = run_scan(sweep_seeds, n_qubits, num_terms, eps, th, eb, leak_bin, t)
                    ranked = top_signatures_by_enrichment(c, top_fraction=0.05, top_k=10)
                    sweep_top[(th, eps, eb)] = [r[0] for r in ranked]
                    print(f"[Sweep] th={th:.2f}, eps={eps:.2f}, ent_bin={eb:.2f} -> candidates={len(c)}")
                    for sig, enr, cs, ca in ranked[:3]:
                        print(f"  sig={sig}  enrichment={enr:.2f}x  stable={cs}  overall={ca}")
                    print()

        avg_j, freq = summarize_consistency(sweep_top, top_k=10)
        print("Robustness summary (top-10 sets):")
        if math.isnan(avg_j):
            print("  Not enough non-empty sweep results to compute overlap.\n")
        else:
            print(f"  Avg pairwise Jaccard overlap: {avg_j:.3f}")
            print("  Most frequent signatures in sweep top-10 (top 10):")
            for sig, cnt in freq[:10]:
                print(f"    sig={sig}  appears_in={cnt} runs")
            print()

        # 2) enrichment (full nominal)
        print("=== 2) Enrichment Test (nominal) + Bootstrap CI ===")
        print(f"Nominal: th={nominal_th:.2f}, eps={nominal_eps:.2f}, ent_bin={nominal_ent_bin:.2f}\n")
        c_full = run_scan(seeds, n_qubits, num_terms, nominal_eps, nominal_th, nominal_ent_bin, leak_bin, t)
        print("Nominal basic stats:")
        print_basic_stats(c_full, n_qubits)
        print()

        for frac in [0.01, 0.05]:
            print(f"Stable set = top {int(frac*100)}% by score")
            ranked = top_signatures_by_enrichment(c_full, top_fraction=frac, top_k=10)
            if not ranked:
                print("  No ranked signatures.\n")
                continue
            for sig, enr, cs, ca in ranked[:5]:
                mean, lo, hi = bootstrap_enrichment_ci(c_full, sig, frac, args.bootstrap, 123)
                ci = "CI unavailable" if math.isnan(mean) else f"bootstrap={mean:.2f}x, 95% CI [{lo:.2f}, {hi:.2f}]"
                print(f"  sig={sig}  point={enr:.2f}x  stable={cs}  overall={ca}  |  {ci}")
            print()

        # 3) hold-out replication (nominal)
        print("=== 3) Hold-out Replication (nominal) ===")
        print(f"Half A seeds={len(seeds_A)} | Half B seeds={len(seeds_B)}\n")
        cA = run_scan(seeds_A, n_qubits, num_terms, nominal_eps, nominal_th, nominal_ent_bin, leak_bin, t)
        cB = run_scan(seeds_B, n_qubits, num_terms, nominal_eps, nominal_th, nominal_ent_bin, leak_bin, t)
        topA = top_signatures_by_enrichment(cA, top_fraction=0.05, top_k=10)
        topB = top_signatures_by_enrichment(cB, top_fraction=0.05, top_k=10)
        setA, setB = set([x[0] for x in topA]), set([x[0] for x in topB])

        if not setA or not setB:
            print("Hold-out: insufficient ranked signatures in one or both halves.\n")
        else:
            inter, union = len(setA & setB), len(setA | setB)
            jacc = inter / union if union else 0.0
            print(f"Top-10 set overlap (top 5% stable enrichment): Jaccard={jacc:.3f} (inter={inter}, union={union})")
            print("Overlapping signatures:")
            for sig in list(setA & setB)[:15]:
                print(f"  sig={sig}")
            print()

        print("=== End of v0_6.1_Final ===")
        print(f"Log written to: {os.path.abspath(args.output)}")


if __name__ == "__main__":
    main()
