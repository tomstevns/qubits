...
def main() -> None:
    ...
    ap.add_argument("--p_dom_thresh", type=float, default=0.25)
    ...
    ap.add_argument("--output", type=str, default="v0_7_baseline_sigV2_matched_FRESH_output.txt")
    args = ap.parse_args()

    # Stable set definition(s)
    stable_fracs = []
    if getattr(args, 'stable_fracs', None):
        try:
            stable_fracs = [float(x.strip()) for x in str(args.stable_fracs).split(',') if x.strip()]
        except Exception:
            stable_fracs = []
    if not stable_fracs:
        stable_fracs = [float(args.stable_frac)]
    stable_fracs = sorted(set(stable_fracs))
    ...
    try:
        ...
        print(f"Evidence: stable_fracs={stable_fracs} | topK={args.topK} | bootstrap={args.bootstrap}")
        ...
        for model in models:
            ...
            if not cands:
                print('No candidates; skipping evidence layer.')
                dt = time.time() - t_model
                print(f"Model runtime: {dt:.1f} s\n")
                continue

            # Evidence layer is computed for one or more stable-set definitions (stable_fracs).
            # Overall counts are computed once per model; stable-set counts are recomputed per stable_frac.
            scores = np.array([c.score for c in cands], dtype=float)

            # Overall fine-grained signature counts (independent of stable_frac).
            fine_overall: Dict[SigKey, int] = {}
            for c in cands:
                fine_overall[c.sig] = fine_overall.get(c.sig, 0) + 1

            for stable_frac in stable_fracs:
                stable_frac = float(stable_frac)
                if not (0.0 < stable_frac < 1.0):
                    print(f'Skipping invalid stable_frac={stable_frac} (must be in (0,1)).')
                    continue

                cutoff = np.quantile(scores, 1.0 - stable_frac)
                stable = [c for c in cands if c.score >= cutoff]
                stable_frac_eff = (len(stable) / max(1, len(cands)))

                fine_stable: Dict[SigKey, int] = {}
                for c in stable:
                    fine_stable[c.sig] = fine_stable.get(c.sig, 0) + 1

                ranked = summarize_family_enrichment(
                    fine_overall,
                    fine_stable,
                ...
                print(f'-- Evidence @ stable_frac={stable_frac:.4f} (effective={stable_frac_eff:.4f}) --')
                if not ranked:
                    print('No signature families passed min_support at this stable_frac.')
                else:
                    for fam, enr, s_cnt, o_cnt in ranked:
                        ci_lo, ci_hi = bootstrap_enrichment_ci(
                            cands,
                            fam,
                        ...
                        print(f'  {fam} | enrichment={enr:.3f}x | stable={s_cnt} / overall={o_cnt} | CI[{ci_lo:.3f}, {ci_hi:.3f}]')

                # Hold-out replication (per stable_frac).
                half = len(cands) // 2
                cA = cands[:half]
                cB = cands[half:]

                def ranks_for_half(c_half, st_frac: float):
                    if len(c_half) < 2:
                        return []
                    sc = np.array([c.score for c in c_half], dtype=float)
                    co = np.quantile(sc, 1.0 - st_frac)
                    st = [c for c in c_half if c.score >= co]
                    fo: Dict[SigKey, int] = {}
                    fs: Dict[SigKey, int] = {}
                    for c in c_half:
                    ...
                    return rank_families(
                        fam_o, fam_s, st_frac,
                        min_support=int(args.min_support),
                        top_k=max(10, int(args.topK)),
                    )

                rankA = ranks_for_half(cA, stable_frac)
                rankB = ranks_for_half(cB, stable_frac)
...
def topk_union_mask(p_i: np.ndarray, p_j: np.ndarray, k: int) -> np.ndarray:
    """Union mask of top-k basis states from each probability vector."""
    k = max(1, int(k))
    idx_i = np.argsort(p_i)[-k:]
    idx_j = np.argsort(p_j)[-k:]
    mask = np.zeros_like(p_i, dtype=bool)
    mask[idx_i] = True
    mask[idx_j] = True
    return mask
