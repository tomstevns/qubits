...
def bootstrap_ci(top_sigs: List[Tuple[int,int,int,int]], sigs: List[Tuple[int,int,int,int]], scores: np.ndarray,
    ...
    # Keep signatures hashable through bootstrapping.
    # np.array(list_of_tuples) can create a 2D array and turn tuples into lists.
    sig_arr = np.empty(len(sigs), dtype=object)
    sig_arr[:] = [tuple(s) for s in sigs]

    store = {s: [] for s in top_sigs}
    for _ in range(B):
        idx = rng.integers(0, n, size=n)
        b_scores = scores[idx]
        b_sigs = [sig_arr[i] for i in idx]
...
def parse_args():
    ...
    p.add_argument("--out", type=str, default="v0_7_baseline_sigV2_matched_FRESH5_output.txt")
