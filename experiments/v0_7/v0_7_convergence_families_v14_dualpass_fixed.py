...
# -*- coding: utf-8 -*-
"""
v0_7_convergence_families_v14.py
...
def stable_mask_from_scores_and_leak(
    ...
    return mask
def stable_mask_from_pooled_thresholds(
    scores: np.ndarray,
    leaks: np.ndarray,
    stable_frac: float,
    score_thr: float,
    stable_leak_max: Optional[float],
    pooled_leak_thr: Optional[float],
) -> np.ndarray:
    """
    Stable mask using a shared (pooled) score threshold across REAL+NULL within a batch.

    - Primary: score >= score_thr (thr computed from pooled scores at quantile 1-stable_frac).
    - Optional leakage constraint:
        * absolute: leak <= stable_leak_max
        * or pooled quantile threshold: leak <= pooled_leak_thr
    - If the resulting mask is empty, fall back to score-only top stable_frac within the model.
    """
    n = scores.size
    if n == 0:
        return np.zeros(0, dtype=bool)
    ...
    if stable_leak_max is not None:
        mask = mask & (leaks <= float(stable_leak_max))
    elif pooled_leak_thr is not None:
        mask = mask & (leaks <= float(pooled_leak_thr))

    if np.any(mask):
        return mask

    # Fallback: model-local score-only stable set
    return stable_mask_from_scores_and_leak(scores, leaks, stable_frac, None, None)
...
def compute_batch_result(
    *,
    ...
    stable_leak_quantile: Optional[float],
    stable_mode: str,
    pooled_score_thr: Optional[float],
    pooled_leak_thr: Optional[float],
    topK: int,
    min_overall: int,
    min_stable: int,
    ...
    if not cands:
        return BatchResult(
            ...
            n_candidates=0, n_stable=0, stable_rate=0.0, stable_rate_scoreonly=0.0,
    ...
    if stable_mode == "pooled" and pooled_score_thr is not None:
        # Use pooled score threshold (shared across REAL+NULL for this batch) to define stable.
        # For a quantile-based leak constraint, we also use a pooled leak threshold (pooled_leak_thr).
        mask = stable_mask_from_pooled_thresholds(scores, leaks, stable_frac, pooled_score_thr, stable_leak_max, pooled_leak_thr)
    else:
        # Model-local: top stable_frac by score (optionally intersected with a leak constraint).
...
def run_paired_batches(
    *,
    ...
    for b in range(n_batches):
        ...
        # Pooled thresholds for stable selection (optional)
        pooled_score_thr = None
        pooled_leak_thr = None
        if stable_mode == "pooled":
            pooled_scores = np.array([c.score for c in real_cands] + [c.score for c in null_cands], dtype=float)
            if pooled_scores.size:
                pooled_score_thr = float(np.quantile(pooled_scores, 1.0 - float(stable_frac)))
            if stable_leak_quantile is not None:
                q = float(stable_leak_quantile)
                q = max(0.0, min(1.0, q))
                pooled_leaks = np.array([c.leak for c in real_cands] + [c.leak for c in null_cands], dtype=float)
                if pooled_leaks.size:
                    pooled_leak_thr = float(np.quantile(pooled_leaks, q))
        ...
        R = compute_batch_result(
            ...
            pooled_score_thr=pooled_score_thr,
            pooled_leak_thr=pooled_leak_thr,
            topK=topK,
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="v0_7 convergence harness (v14 dual-pass): runs PASS A and PASS B in one execution and writes a single consolidated output file."
    )
    ...
    # Legacy / single-pass knobs (kept for backward compatibility)
    ap.add_argument(
        "--stable_mode",
        type=str,
        default="model",
        choices=["model", "pooled"],
        help="Stable selection mode. 'model' uses model-local score quantile. 'pooled' uses pooled score quantile across REAL+NULL per batch.",
    )
    ...
    ap.add_argument(
        "--stable_leak_quantile",
        type=float,
        default=None,
        help="Optional: additionally require leak <= quantile(leak, q) for stable candidates (q in [0,1]).",
    )
    ...
    # Dual-pass driver (new default behavior)
    ap.add_argument(
        "--single_pass",
        action="store_true",
        help="Run only one pass using the explicit --stable_* settings (legacy behavior).",
    )
    ap.add_argument(
        "--dual_leak_max",
        type=float,
        default=0.05,
        help="In dual-pass mode, PASS B enforces leak <= dual_leak_max in addition to score (pooled) selection.",
    )

    ap.add_argument("--output", type=str, default="v0_7_convergence_families_v14_output.txt")
    args = ap.parse_args()

    def build_header(pass_mode: str) -> str:
        header = []
        header.append("=== v0_7: Convergence + Baseline Calibration (v14 dual-pass) ===")
        header.append(f"Mode: {pass_mode}")
        header.append(f"Qubits: {args.n_qubits} (d={2**args.n_qubits}) | terms={args.n_terms}")
        header.append(
            f"Batches: {args.batches} × {args.seeds_per_batch} seeds (base_seed={args.base_seed}, stride={args.batch_stride})"
        )
        ...
        header.append(
            f"Bins (SigQG_COARSE): q_bins_coarse={args.q_bins_coarse} + dom_bin + leak_bin_coarse (pooled REAL+NULL per batch)"
        )
        ...
        header.append(
            f"Paired NN fallback (fine): {bool(args.paired_nn)} (weights dom/e1/e2/leak={args.nn_w_dom}/{args.nn_w_e1}/{args.nn_w_e2}/{args.nn_w_leak})"
        )
        header.append("")
        return "\n".join(header)

    def emit_pass_report(
        out_fn,
        pass_label: str,
        real_res: List[BatchResult],
        null_res: List[BatchResult],
        scoreboards: List[Dict[str, float]],
        nn_fine: bool,
    ) -> None:
        ...
        for b in range(args.batches):
            out_fn("----------------------------------------------")
            out_fn(f"Batch {b+1}/{args.batches} (seed_offset={real_res[b].seed_offset})")

            R = real_res[b]
            out_fn(
                f"Model: REAL | candidates={R.n_candidates} | stable={R.n_stable} | stable_rate={R.stable_rate:.4f} (score-only ref={R.stable_rate_scoreonly:.4f})"
            )
            out_fn(f"  score(mean/median/max)={R.score_stats[0]:.3f}/{R.score_stats[1]:.3f}/{R.score_stats[2]:.3f}")
            out_fn(f"  leakage(mean/median/min)={R.leak_stats[0]:.3f}/{R.leak_stats[1]:.3f}/{R.leak_stats[2]:.3f}")
            out_fn(f"  entropy(mean/median/max)={R.ent_stats[0]:.3f}/{R.ent_stats[1]:.3f}/{R.ent_stats[2]:.3f}")
            out_fn(f"  dom_count(mean/median/max)={R.dom_stats[0]:.2f}/{R.dom_stats[1]:.1f}/{R.dom_stats[2]:.0f}")
            out_fn(f"  entropy effect (stable-overall)={R.effect_entropy_bits:+.3f} bits  CI95={R.effect_ci}")
            out_fn("")
            out_fn(format_top(R.top_qg, "REAL, SigQ_GLOBAL (fine)"))
            out_fn("")
            out_fn(format_top(R.top_qg_coarse, "REAL, SigQG_COARSE"))
            out_fn("")

            N = null_res[b]
            out_fn(
                f"Model: NULL_HAAR_BASIS | candidates={N.n_candidates} | stable={N.n_stable} | stable_rate={N.stable_rate:.4f} (score-only ref={N.stable_rate_scoreonly:.4f})"
            )
            out_fn(f"  score(mean/median/max)={N.score_stats[0]:.3f}/{N.score_stats[1]:.3f}/{N.score_stats[2]:.3f}")
            out_fn(f"  leakage(mean/median/min)={N.leak_stats[0]:.3f}/{N.leak_stats[1]:.3f}/{N.leak_stats[2]:.3f}")
            out_fn(f"  entropy(mean/median/max)={N.ent_stats[0]:.3f}/{N.ent_stats[1]:.3f}/{N.ent_stats[2]:.3f}")
            out_fn(f"  dom_count(mean/median/max)={N.dom_stats[0]:.2f}/{N.dom_stats[1]:.1f}/{N.dom_stats[2]:.0f}")
            out_fn(f"  entropy effect (stable-overall)={N.effect_entropy_bits:+.3f} bits  CI95={N.effect_ci}")
            out_fn("")
            out_fn(format_top(N.top_qg, "NULL, SigQ_GLOBAL (fine)"))
            out_fn("")
            out_fn(format_top(N.top_qg_coarse, "NULL, SigQG_COARSE"))
            out_fn("")

            ov_fine = jaccard(R.top_keys_qg, N.top_keys_qg)
            ov_coarse = jaccard(R.top_keys_qg_coarse, N.top_keys_qg_coarse)
            out_fn(
                f"Batch {b+1}: Jaccard(Top-{args.topK}) REAL vs NULL | fine(SigQ_GLOBAL)={ov_fine:.3f} | coarse(SigQG_COARSE)={ov_coarse:.3f}"
            )
            out_fn("")

            # Paired views (fine: optionally NN; coarse: exact only)
            out_fn(
                format_paired_exact_nn(
            ...
            sb = scoreboards[b]
            out_fn("Baseline scoreboard (REAL - NULL):")
            out_fn(f"  delta median entropy (bits): {sb['delta_median_entropy_bits']:+.3f}")
            out_fn(f"  delta median leakage       : {sb['delta_median_leak']:+.3f}")
            out_fn(f"  delta median dom_count     : {sb['delta_median_dom']:+.3f}")
            out_fn(f"  entropy Cohen's d          : {sb['entropy_cohens_d']:+.3f}")
            out_fn("")

        out_fn("----------------------------------------------")
        out_fn("=== Convergence diagnostics (REAL) ===")
        real_sets_fine = [set(r.top_keys_qg) for r in real_res]
        real_sets_coarse = [set(r.top_keys_qg_coarse) for r in real_res]
        for i in range(len(real_sets_fine)):
            for j in range(i + 1, len(real_sets_fine)):
                out_fn(
                    f"REAL overlap fine:   Jaccard(Top-{args.topK}) batch{i+1} vs batch{j+1} = {jaccard(real_sets_fine[i], real_sets_fine[j]):.3f}"
                )
                out_fn(
                    f"REAL overlap coarse: Jaccard(Top-{args.topK}) batch{i+1} vs batch{j+1} = {jaccard(real_sets_coarse[i], real_sets_coarse[j]):.3f}"
                )

        out_fn("")

    # Run
    pass_mode = "SINGLE-PASS (legacy)" if args.single_pass else "DUAL-PASS (A then B, consolidated)"
    header_text = build_header(pass_mode)
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        ...
        if args.single_pass:
            # Legacy behavior: one pass using user-provided stable_* and paired_nn flag
            ...
            emit_pass_report(
                out_fn=out,
                pass_label=f"PASS (single): stable_mode={args.stable_mode}, leak_max={args.stable_leak_max}, leak_q={args.stable_leak_quantile}, paired_nn={bool(args.paired_nn)}",
                real_res=real,
                null_res=null,
                scoreboards=scoreboards,
                nn_fine=bool(args.paired_nn),
            )
            out("=== End of v14 dual-pass ===")
            return

        # Dual-pass (default): PASS A and PASS B
        out("Dual-pass definitions:")
        out("  PASS A: --stable_mode pooled --paired_nn (no leak constraint)")
        out(f"  PASS B: --stable_mode pooled --paired_nn --stable_leak_max {args.dual_leak_max}")
        out("")

        # PASS A
        real_A, null_A, sb_A = run_paired_batches(
            n_qubits=args.n_qubits,
            n_terms=args.n_terms,
            seeds_per_batch=args.seeds_per_batch,
            n_batches=args.batches,
            ...
            keep_mass=args.keep_mass,
            stable_frac=args.stable_frac,
            stable_leak_max=None,
            stable_leak_quantile=None,
            stable_mode="pooled",
            topK=args.topK,
            min_overall=args.min_overall,
            min_stable=args.min_stable,
        ...
        emit_pass_report(
            out_fn=out,
            pass_label="PASS A (pooled stable, no leak constraint) | paired_nn=True",
            real_res=real_A,
            null_res=null_A,
            scoreboards=sb_A,
            nn_fine=True,
        ...
        # PASS B
        real_B, null_B, sb_B = run_paired_batches(
            n_qubits=args.n_qubits,
            n_terms=args.n_terms,
            seeds_per_batch=args.seeds_per_batch,
            n_batches=args.batches,
            ...
            keep_mass=args.keep_mass,
            stable_frac=args.stable_frac,
            stable_leak_max=float(args.dual_leak_max),
            stable_leak_quantile=None,
            stable_mode="pooled",
            topK=args.topK,
            min_overall=args.min_overall,
        ...
        emit_pass_report(
            out_fn=out,
            pass_label=f"PASS B (pooled stable + leak_max={args.dual_leak_max}) | paired_nn=True",
            real_res=real_B,
            null_res=null_B,
            scoreboards=sb_B,
            nn_fine=True,
        ...
        # Comparative summary
        out("############################################################")
        out("### COMPARATIVE SUMMARY: PASS A vs PASS B")
        out("############################################################")
        out("Interpretation: PASS B is PASS A with an added physical constraint (leak <= dual_leak_max) inside the pooled-stable selection.")
        out("")

        for b in range(args.batches):
            out("----------------------------------------------")
            out(f"Batch {b+1}/{args.batches} (seed_offset={real_A[b].seed_offset})")
            RA, RB = real_A[b], real_B[b]
            NA, NB = null_A[b], null_B[b]

            out(f"REAL stable_rate: A={RA.stable_rate:.4f}  B={RB.stable_rate:.4f}  (stable n: A={RA.n_stable}, B={RB.n_stable})")
            out(f"NULL stable_rate: A={NA.stable_rate:.4f}  B={NB.stable_rate:.4f}  (stable n: A={NA.n_stable}, B={NB.n_stable})")

            out(f"REAL leak median: A={RA.leak_stats[1]:.3f}  B={RB.leak_stats[1]:.3f}")
            out(f"NULL leak median: A={NA.leak_stats[1]:.3f}  B={NB.leak_stats[1]:.3f}")

            out(f"REAL ent median : A={RA.ent_stats[1]:.3f}  B={RB.ent_stats[1]:.3f}")
            out(f"NULL ent median : A={NA.ent_stats[1]:.3f}  B={NB.ent_stats[1]:.3f}")

            # Within-model stability across passes
            out(f"REAL Top-{args.topK} overlap (fine):   Jaccard(A,B)={jaccard(set(RA.top_keys_qg), set(RB.top_keys_qg)):.3f}")
            out(f"REAL Top-{args.topK} overlap (coarse): Jaccard(A,B)={jaccard(set(RA.top_keys_qg_coarse), set(RB.top_keys_qg_coarse)):.3f}")
            out(f"NULL Top-{args.topK} overlap (fine):   Jaccard(A,B)={jaccard(set(NA.top_keys_qg), set(NB.top_keys_qg)):.3f}")
            out(f"NULL Top-{args.topK} overlap (coarse): Jaccard(A,B)={jaccard(set(NA.top_keys_qg_coarse), set(NB.top_keys_qg_coarse)):.3f}")

            # Cross-model separation in each pass
            out(f"Cross-model overlap A: fine={jaccard(set(RA.top_keys_qg), set(NA.top_keys_qg)):.3f}  coarse={jaccard(set(RA.top_keys_qg_coarse), set(NA.top_keys_qg_coarse)):.3f}")
            out(f"Cross-model overlap B: fine={jaccard(set(RB.top_keys_qg), set(NB.top_keys_qg)):.3f}  coarse={jaccard(set(RB.top_keys_qg_coarse), set(NB.top_keys_qg_coarse)):.3f}")

            out("")

        out("=== Notes (scientific reading) ===")
        out("1) PASS A tells you whether pooled-stable alone makes paired 'stable' counts non-zero (comparability fix).")
        out("2) PASS B tells you how much of PASS A's signal survives a physical leakage constraint.")
        out("3) If PASS A yields dst stable > 0 but PASS B collapses to 0, the cross-model comparability exists but is leakage-fragile.")
        out("4) If both PASS A and PASS B yield dst stable > 0 with similar families, you have a robust cross-model mapping.")
        out("")
        out("=== End of v14 dual-pass ===")
