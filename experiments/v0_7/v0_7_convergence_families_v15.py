...
# -*- coding: utf-8 -*-
"""
v0_7_convergence_families_v15.py
...
def stable_mask_from_scores_and_leak(
    ...
    """
    V15 (model-fixed stable fraction):
    ...
    If a leakage constraint is provided:
      ...
      - If eligible count < k: pick all eligible, then *fill the remainder* with the next-best
        items by (score, leak) from the ineligible set. This preserves a fixed stable fraction
        per model while biasing toward low leakage when possible.

    This function returns a boolean mask of length n.
    """
    n = int(scores.size)
    if n == 0:
        return np.zeros((0,), dtype=bool)

    # k stable items (fixed fraction)
    stable_frac = float(stable_frac)
    stable_frac = max(0.0, min(1.0, stable_frac))
    k = int(round(stable_frac * n))
    k = max(1, min(n, k))

    # Order by score primary, leak secondary (lower is better for both)
    order = np.lexsort((leaks, scores))

    # No leak constraint: take top-k by score
    ...
    if len(eligible_order) >= k:
        ...
        chosen = eligible_order[:]  # take all eligible
        need = k - len(chosen)
        if need > 0:
            # Fill from ineligible by (score, leak) ordering (already in 'order')
            for i in order:
                if eligible[i]:
                    continue
                chosen.append(i)
...
class BatchResult:
    ...
    stable_target_k: int
    stable_leak_thr_used: Optional[float]
    stable_eligible: int
    stable_ineligible_used: int
    ...
    *,
    ...
    if not cands:
        return BatchResult(
            ...
            stable_target_k=0, stable_leak_thr_used=None, stable_eligible=0, stable_ineligible_used=0,
    ...
    if stable_mode == "pooled" and pooled_score_thr is not None:
        ...
        # Model-fixed: select exactly stable_frac per model (optionally with leak constraint + guardrails).
        mask = stable_mask_from_scores_and_leak(scores, leaks, stable_frac, stable_leak_max, stable_leak_quantile)

    # Diagnostics for fixed-fraction stable selection
    n_events = int(len(cands))
    stable_frac_clamped = max(0.0, min(1.0, float(stable_frac)))
    stable_target_k = int(round(stable_frac_clamped * n_events)) if n_events > 0 else 0
    stable_target_k = max(1, min(n_events, stable_target_k)) if n_events > 0 else 0

    stable_leak_thr_used: Optional[float] = None
    stable_eligible = n_events
    stable_ineligible_used = 0

    if (stable_leak_max is not None) or (stable_leak_quantile is not None):
        if stable_leak_max is not None:
            stable_leak_thr_used = float(stable_leak_max)
        else:
            q = float(stable_leak_quantile) if stable_leak_quantile is not None else 1.0
            q = max(0.0, min(1.0, q))
            stable_leak_thr_used = float(np.quantile(leaks, q))
        eligible_mask = leaks <= stable_leak_thr_used
        stable_eligible = int(np.sum(eligible_mask))
        stable_ineligible_used = int(np.sum(mask & (~eligible_mask)))
    ...
    return BatchResult(
        ...
        stable_target_k=stable_target_k, stable_leak_thr_used=stable_leak_thr_used, stable_eligible=stable_eligible, stable_ineligible_used=stable_ineligible_used,
...
def main() -> None:
    ...
    ap.add_argument("--stable_mode", type=str, default="model", choices=["model_fixed", "model", "pooled"], help="Stable selection mode. model=top stable_frac per model (default). pooled=use pooled score threshold across REAL+NULL per batch (more comparable in paired views).")
    ...
    ap.add_argument("--output", type=str, default="v0_7_convergence_families_v15_output.txt")
    args = ap.parse_args()

    # Backward-compat alias
    if args.stable_mode == "model":
        args.stable_mode = "model_fixed"

    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration (v15) ===")
    ...
    header.append(f"Stable selection: stable_frac={args.stable_frac:.3f} mode={args.stable_mode} with optional leak constraint (max={args.stable_leak_max}, q={args.stable_leak_quantile})")
