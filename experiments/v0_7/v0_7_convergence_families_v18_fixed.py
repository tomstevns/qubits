...
def _unitary_from_hermitian(K: np.ndarray, scale: float) -> np.ndarray:
    """Return U = exp(-i * scale * K) for Hermitian K using eigendecomposition (no SciPy dependency)."""
    # Ensure Hermitian
    K = 0.5 * (K + K.conj().T)
    # Normalize to keep 'scale' meaningful across random draws
    fro = float(np.linalg.norm(K, ord="fro"))
    if fro > 0.0:
        K = K / fro
    w, V = np.linalg.eigh(K)
    phase = np.exp(-1j * float(scale) * w)
...
def perturbation_robustness_summary_for_batch(
    *,
    ...
    For each perturbation, NULL uses the perturbed eigenvalues (spectrum-matched). The eigenbasis is either
    (a) freshly resampled Haar-random per perturbation (null_perturb_mode='haar_resample'), or
    (b) a *correlated* random basis obtained by applying a small random unitary rotation to the baseline Haar basis
        (null_perturb_mode='local_unitary'). The latter makes drift-style robustness metrics (e.g., |Δentropy|)
        comparable between REAL and NULL, because both models undergo small, controlled changes rather than full resampling.
    ...
    for s in range(int(n_seeds)):
        ...
        for eta in eta_list:
            ...
            for rep in range(int(reps)):
                ...
                # NULL under perturbation: spectrum-matched eigenvalues with controlled eigenbasis change
                if null_perturb_mode == "haar_resample":
                    tag = f"NULL_HAAR_BASIS_PERT|eta{eta:.6g}|rep{rep}"
                    _, evecs_null_p = null_haar_basis_eigs(evals_p, seed=seed, tag=tag)
                else:
                    # 'local_unitary': apply a small random unitary rotation to the baseline Haar basis
                    seed_rot = stable_hash_int(f"NULL_ROT|{seed}|eta{eta:.6g}|rep{rep}")
                    K = build_random_pauli_hamiltonian_cached(cache, perturb_terms_eff, seed_rot)
                    U_delta = _unitary_from_hermitian(K, scale=eta)
                    evecs_null_p = U_delta @ evecs_null_base
    ...
    for b in range(n_batches):
        ...
        if perturb_eta and int(perturb_seeds) > 0:
            ...
            pert_summary = perturbation_robustness_summary_for_batch(
                ...
                null_perturb_mode=null_perturb_mode,
                eps_neighbor=eps_neighbor,
                times=times,
                keep_mass=keep_mass,
                ent_step=ent_step,
...
def main() -> None:
    ...
    ap.add_argument("--null_perturb_mode", choices=["local_unitary","haar_resample"], default="local_unitary",
                    help="NULL perturbation basis mode: local_unitary (correlated small rotation; recommended for drift metrics) or haar_resample (fresh Haar basis per perturbation).")
    ...
    ap.add_argument("--output", type=str, default="v0_7_convergence_families_v18_fixed_output.txt")
    args = ap.parse_args()
    ...
    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration + Perturbation Robustness (v18) ===")
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        ...
        real, null, scoreboards, perturb_summaries = run_paired_batches(
            ...
            null_perturb_mode=args.null_perturb_mode,
