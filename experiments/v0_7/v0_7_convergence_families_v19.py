...
- Perturbation robustness (v19): apply the *same* small Hermitian perturbation ΔH in the
    ...
    for s in range(int(n_seeds)):
        ...
        for model, evecs in [("REAL", evecs_real_base), ("NULL", evecs_null_base)]:
            ...
            for (i, j, _de) in pairs:
                ...
                feats.append((i, j, ent_i, ent_j, dom, leak, sig_c, score, U))
            feats.sort(key=lambda t: t[-2])  # lower score = more "stable/interesting"
            take = int(max(1, min(int(pairs_per_seed), len(feats))))
            base_lists[model] = [(i, j, ent_i, ent_j, dom, leak, sig_c, U) for (i, j, ent_i, ent_j, dom, leak, sig_c, _score, U) in feats[:take]]
        ...
        for eta in eta_list:
            ...
            for rep in range(int(reps)):
                ...
                for model, evals_p, evecs_p, base_items in [
                    ...
                    for (i, j, ent_i0, ent_j0, _dom0, leak0, sig0, U0) in base_items:
...
def main() -> None:
    ...
    ap.add_argument("--output", type=str, default="v0_7_convergence_families_v19_output.txt")
    args = ap.parse_args()
    ...
    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration + Perturbation Robustness (v19) ===")
