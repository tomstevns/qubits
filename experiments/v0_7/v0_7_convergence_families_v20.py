...
# -*- coding: utf-8 -*-
"""\
v0_7_convergence_families_v20.py

Purpose
-------
Evidence step v20: add a physically meaningful, basis-invariant perturbation robustness test.
...
- Perturbation robustness (v20): apply the *same* small Hermitian perturbation ΔH in the
...
class Candidate:
    ...
    j: int
    delta_e: float                 # ΔE_in
    gap_out: float                 # ΔE_out (nearest external gap)
    iso_ratio: float               # R = ΔE_out / ΔE_in
    ent_i: float
    ent_j: float
    dom: int
    leak: float
...
def out_gap_for_neighbor_pair(evals: np.ndarray, i: int) -> float:
    """    For a neighbor pair (i, i+1) in a sorted spectrum evals, compute the closest
    *external* spectral gap to the rest of the spectrum.

    We approximate ΔE_out as the minimum of the adjacent gaps flanking the pair:
      left  = E[i]   - E[i-1]   (if i>0)
      right = E[i+2] - E[i+1]   (if i+2<d)

    Returns +inf at spectrum edges where only one side exists.
    """
    d = int(evals.size)
    i = int(i)
    if d < 2:
    ...
    right = float("inf")
    if i - 1 >= 0:
        left = float(abs(float(evals[i]) - float(evals[i - 1])))
    if i + 2 < d:
        right = float(abs(float(evals[i + 2]) - float(evals[i + 1])))
    return float(min(left, right))


def isolation_ratio_for_neighbor_pair(evals: np.ndarray, i: int, eps: float = 1e-15) -> float:
    """Isolation ratio R = ΔE_out / ΔE_in for neighbor pair (i, i+1)."""
    d_in = float(abs(float(evals[i + 1]) - float(evals[i])))
    d_out = out_gap_for_neighbor_pair(evals, i)
    if d_in <= float(eps):
        return float("inf")
    return float(d_out / d_in)


def safe_median_finite(x: np.ndarray) -> float:
    x = np.asarray(x, dtype=float)
    if x.size == 0:
        return 0.0
    xf = x[np.isfinite(x)]
...
def generate_candidates_for_seed(
    ...
    for (i, j, de) in pairs:
        gap_out = out_gap_for_neighbor_pair(evals, i)
        iso_ratio = (gap_out / max(1e-12, float(de))) if np.isfinite(gap_out) else float('inf')
...
def perturbation_robustness_summary_for_batch(
    *,
    ...
    v20 change: Robustness is quantified with a *basis-invariant* 2D subspace overlap.
    ...
    for s in range(int(n_seeds)):
        ...
        for model, evecs in [("REAL", evecs_real_base), ("NULL", evecs_null_base)]:
            ...
            for (i, j, _de) in pairs:
                ...
                iso0 = isolation_ratio_for_neighbor_pair(evals_base, i)
                feats.append((i, j, ent_i, ent_j, dom, leak, sig_c, score, U, iso0))
            feats.sort(key=lambda t: t[7])  # lower score = more "stable/interesting"
            take = int(max(1, min(int(pairs_per_seed), len(feats))))
            base_lists[model] = [(i, j, ent_i, ent_j, dom, leak, sig_c, U, iso0) for (i, j, ent_i, ent_j, dom, leak, sig_c, _score, U, iso0) in feats[:take]]
        ...
        for eta in eta_list:
            ...
            for rep in range(int(reps)):
                ...
                for model, evals_p, evecs_p, base_items in [
                    ...
                    for (i, j, ent_i0, ent_j0, _dom0, leak0, sig0, U0, iso0) in base_items:
                        ...
                        acc["iso_all"].append(float(iso0))
                        if retained:
                            acc["ov_ret"].append(float(ov))
                            acc["iso_ret"].append(float(iso0))
    ...
    for eta in eta_list:
        ...
        for model in ["REAL", "NULL"]:
            ...
            iso_all = list(acc.get("iso_all", []))
            iso_ret = list(acc.get("iso_ret", []))
            iso_ret_f = [x for x in iso_ret if np.isfinite(x)]

            acc["iso_ratio_retained_p10"] = _q(iso_ret_f, 0.10)
            acc["iso_ratio_retained_p50"] = _q(iso_ret_f, 0.50)
            acc["iso_ratio_retained_p90"] = _q(iso_ret_f, 0.90)

            # Correlation between isolation ratio and subspace overlap among retained pairs (Pearson r)
            # (finite-only, pairwise aligned by construction)
            if len(iso_ret_f) >= 3 and len(ov_ret) == len(iso_ret):
                a = np.array([x for x in iso_ret if np.isfinite(x)], dtype=float)
                b = np.array([ov_ret[i] for i in range(len(ov_ret)) if np.isfinite(iso_ret[i])], dtype=float)
                if a.size >= 3 and b.size == a.size:
                    ma = float(np.mean(a)); mb = float(np.mean(b))
                    va = float(np.var(a)); vb = float(np.var(b))
                    if va > 1e-15 and vb > 1e-15:
                        acc["corr_iso_vs_overlap_retained"] = float(np.mean((a - ma) * (b - mb)) / math.sqrt(va * vb))
                    else:
                        acc["corr_iso_vs_overlap_retained"] = 0.0
                else:
                    acc["corr_iso_vs_overlap_retained"] = 0.0
            else:
                acc["corr_iso_vs_overlap_retained"] = 0.0
            for k in ["pairs", "retained", "sig_retained", "d_ent", "d_leak", "ov_all", "ov_ret", "iso_all", "iso_ret"]:
                acc.pop(k, None)
...
class BatchResult:
    ...
    gap_in_stats: Tuple[float, float, float]  # mean, median, max (ΔE_in)
    gap_out_stats: Tuple[float, float, float]  # mean, median, max (ΔE_out)
    iso_stats: Tuple[float, float, float]  # mean, median, p90 (finite only)
    ent_pool: np.ndarray
    leak_vals: np.ndarray
    dom_vals: np.ndarray
    gap_in_vals: np.ndarray
    gap_out_vals: np.ndarray
    iso_vals: np.ndarray
    top_qg: List[FamRow]
    ...
    *,
    ...
    if not cands:
        return BatchResult(
            ...
            iso_stats=(0.0, 0.0, 0.0),
            ent_pool=np.array([], dtype=float),
            leak_vals=np.array([], dtype=float),
            dom_vals=np.array([], dtype=float),
            gap_in_vals=np.array([], dtype=float),
            gap_out_vals=np.array([], dtype=float),
            iso_vals=np.array([], dtype=float),
    ...
    iso = np.array([c.iso_ratio for c in cands], dtype=float)
    ...
    return BatchResult(
        ...
        gap_in_stats=(float(gap_in.mean()), float(np.median(gap_in)), float(gap_in.max())),
        gap_out_stats=(float(gap_out[np.isfinite(gap_out)].mean()) if np.any(np.isfinite(gap_out)) else 0.0,
                      safe_median_finite(gap_out),
                      float(np.max(gap_out[np.isfinite(gap_out)])) if np.any(np.isfinite(gap_out)) else 0.0),
        iso_stats=(float(iso[np.isfinite(iso)].mean()) if np.any(np.isfinite(iso)) else 0.0,
                  safe_median_finite(iso),
                  float(np.quantile(iso[np.isfinite(iso)], 0.90)) if np.any(np.isfinite(iso)) else 0.0),
        ent_pool=ent_pool,
        leak_vals=leaks,
        dom_vals=doms,
        gap_in_vals=gap_in,
        gap_out_vals=gap_out,
        iso_vals=iso,
        top_qg=top_qg,
...
def run_paired_batches(
    *,
    ...
    for b in range(n_batches):
        ...
        # v20: perturbation robustness (optional; uses only a small prefix of seeds to control runtime)
        ...
        sb["delta_median_iso_ratio"] = safe_median_finite(R.iso_vals) - safe_median_finite(N.iso_vals)
        sb["delta_median_gap_in"] = safe_median(R.gap_in_vals) - safe_median(N.gap_in_vals)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="v0_7 convergence harness v20 (v16 baseline + evidence-grade perturbation robustness; n_qubits=4 default).")
    ...
    # v20: perturbation robustness controls (disabled unless --perturb_eta is provided and --perturb_seeds > 0)
    ...
    ap.add_argument("--output", type=str, default="v0_7_convergence_families_v20_output.txt")
    args = ap.parse_args()
    ...
    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration + Perturbation Robustness + Isolation Metrics (v20) ===")
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        ...
        for b in range(args.batches):
            ...
            out(f"  gaps: ΔE_in(mean/med/max)={R.gap_in_stats[0]:.4f}/{R.gap_in_stats[1]:.4f}/{R.gap_in_stats[2]:.4f} | ΔE_out(mean/med/max)={R.gap_out_stats[0]:.4f}/{R.gap_out_stats[1]:.4f}/{R.gap_out_stats[2]:.4f} | iso_ratio(mean/med/p90)={R.iso_stats[0]:.2f}/{R.iso_stats[1]:.2f}/{R.iso_stats[2]:.2f}")
            ...
            out(f"  gaps: ΔE_in(mean/med/max)={N.gap_in_stats[0]:.4f}/{N.gap_in_stats[1]:.4f}/{N.gap_in_stats[2]:.4f} | ΔE_out(mean/med/max)={N.gap_out_stats[0]:.4f}/{N.gap_out_stats[1]:.4f}/{N.gap_out_stats[2]:.4f} | iso_ratio(mean/med/p90)={N.iso_stats[0]:.2f}/{N.iso_stats[1]:.2f}/{N.iso_stats[2]:.2f}")
            ...
            # v20: Perturbation robustness summary (optional)
                ...
                for eta in sorted(ps.keys()):
                    ...
                    for model in ["REAL", "NULL"]:
                        ...
                        out(
                            ...
                            f"iso_ret(p10/p50/p90)={mtr['iso_ratio_retained_p10']:.2f}/{mtr['iso_ratio_retained_p50']:.2f}/{mtr['iso_ratio_retained_p90']:.2f} | "
                            f"corr(iso,ov)_ret={mtr['corr_iso_vs_overlap_retained']:+.3f} | "
            ...
            out(f"  delta median iso_ratio     : {sb['delta_median_iso_ratio']:+.2f}")
            out(f"  delta median ΔE_in         : {sb['delta_median_gap_in']:+.4f}")
            out("")
        ...
        out("4) Isolation ratio R=ΔE_out/ΔE_in helps separate 'spectral neighbor persistence' from 'geometric subspace stiffness' under perturbations.")
        out("")
        out("=== End of v20 ===")
