...
# -*- coding: utf-8 -*-
"""\
v0_7_convergence_families_v19.py
...
- Perturbation robustness (v21.1) + Open-system (v22): apply the *same* small Hermitian perturbation ΔH in the
...
def open_system_summary_for_batch(
    *,
    ...
    Ls, LLs = build_noise_operators(cache.n_qubits, noise_model, gamma_phi, gamma_1, None)
...
def main() -> None:
    ...
    ap.add_argument("--output", type=str, default="v0_7_convergence_families_v22_output.txt")
    args = ap.parse_args()
    ...
    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration + Perturbation Robustness + Gap/Isolation + Open-system (v22) ===")
    ...
    header.append(f"Open-system (v22): enabled={bool(args.open_system)} | include_baselines={bool(args.os_include_baselines)} | pairs_per_batch={args.os_pairs_per_batch} | noise={args.os_noise_model} | gamma_phi={args.os_gamma_phi} | gamma_1={args.os_gamma_1} | t_max={args.os_t_max} | t_steps={args.os_t_steps} | states={args.os_states_mode} | stable_pool={bool(args.os_use_stable_pool)}")
    ...
    iso_eps=args.iso_eps,
        ...
        for b in range(args.batches):
            ...
            # Open-system (v22) summary (optional)
            if bool(args.open_system):
                ...
                if isinstance(osb, dict) and meta:
                    out("Open-system logical retention (Lindblad; v22):")
                    out(
                    ...
                    def _fmt_cat(cat: str) -> None:
                        ...
                        out(f"    AUC medians: fid_uncond={fu.get('auc_med', float('nan')):.3f} | leak={leak.get('auc_med', float('nan')):.3f}")
